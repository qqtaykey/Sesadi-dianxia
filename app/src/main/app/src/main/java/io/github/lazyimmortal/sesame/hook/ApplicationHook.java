...
public class ApplicationHook implements IXposedHookLoadPackage {
    ...
    /**
     * 创建桌面快捷方式
     * 在LSPatch集成模式下使用，确保安装后能在桌面找到应用
     */
    private static void createDesktopShortcut() {
        try {
            if (context == null) {
                Log.i(TAG, "创建桌面快捷方式失败：context为null");
                return;
            }
            
            // 检查是否已存在快捷方式
            if (hasShortcut(context)) {
                Log.i(TAG, "桌面快捷方式已存在，跳过创建");
                return;
            }
            ...
            // 设置快捷方式名称
            shortcutIntent.putExtra(Intent.EXTRA_SHORTCUT_NAME, "芝麻开门");
            
            // 避免重复创建
            shortcutIntent.putExtra("duplicate", false);
            
            // 设置快捷方式图标
            Intent intent = new Intent(context, io.github.lazyimmortal.sesame.ui.MainActivity.class);
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                shortcutIntent.putExtra(Intent.EXTRA_SHORTCUT_ICON, context.getResources().getDrawable(iconResId));
            }
            
            // 发送广播创建快捷方式
            context.sendBroadcast(shortcutIntent);
            Log.record("已创建桌面快捷方式");
        } catch (Exception e) {
            Log.i(TAG, "创建桌面快捷方式失败:");
            Log.printStackTrace(TAG, e);
        }
    }
    ...
    private static boolean hasShortcut(Context context) {
        try {
            ...
            Cursor cursor = resolver.query(uri, new String[]{"title"}, "title=?", new String[]{"芝麻开门"}, null);
    ...
    private synchronized Boolean initHandler(Boolean force) {
        ...
        try {
            if (force) {
                ...
                // 创建桌面快捷方式，确保在LSPatch集成模式下能在桌面找到应用
    ...
    private void initSimplePageManager() {
        ...
        // 注册主页面处理器，监听"便捷生活 一点就好"文字点击
        enableWindowMonitoring(classLoader);
        addHandler("com.eg.android.AlipayGphone.MainActivity", new MainPageHandler());
    }
